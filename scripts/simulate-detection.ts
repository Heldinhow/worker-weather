// Usage: bun scripts/simulate-detection.ts --icao SBGR --obs 19.5 --initial-max 17.0
//
// Fetches today's real buckets from Polymarket Gamma API, then fires handleObs
// with a synthetic observation so the full detection → trade path runs against
// the live CLOB. Safe to run with no wallet balance or on a zeroed-out bucket
// — postOrder will attempt and log the CLOB error gracefully.
...
async function fetchBuckets(citySlug: string): Promise<BucketState[]> {
  const slug = todaySlug(citySlug);
...
function parseArgs(): { icao: string; obs: number; initialMax: number } {
  ...
  if (!icao || !obsStr || !initialMaxStr) {
    console.error("Usage: bun scripts/simulate-detection.ts --icao SBGR --obs 19.5 --initial-max 17.0");
    process.exit(1);
  }

  return { icao, obs: parseFloat(obsStr), initialMax: parseFloat(initialMaxStr) };
}

const { icao, obs, initialMax } = parseArgs();
...
console.log(`[sim] fetching buckets for ${city.slug} (${icao})...`);
const buckets = await fetchBuckets(city.slug);
